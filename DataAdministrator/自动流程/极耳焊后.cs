...
namespace DataAdministrator.自动流程
{
    class 极耳焊后 : Father
    {
        public 极耳焊后(Form1 form, string FilePath2)
        {
            this.form = form;
            FilePath = FilePath2;
            MES_operator = "user";
        ...
        public override string 工位1进站(string Code = "")
        {
            ...
            // 信号的上升延
            form.P_Class.TxtLog("接收极耳焊后进站1信号，开始进站！", 2);
            moduleCode1 = form.PLC_ui.Read_String(FindIniPath("WIP码")[0]);
            form.P_Class.TxtLog("极耳焊后1进站WIP码：" + moduleCode1, 2);
            ...
            {
                string url = getData2Value("极耳焊后URL");
                string deviceCode = getData2Value("极耳焊后设备编码");
                string processedMode = getData2Value("极耳焊后加工模式");
                ...
                string userOperator = getData2Value("极耳焊后操作人");
                string timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                if (mesClass.inbound_MES(url, moduleCode1, deviceCode, processedMode, userOperator, timestamp, 2, "进站1OK", "进站1NG",ref hanHoProductCode))
                {
                    form.Invoke(new MethodInvoker(delegate
                    {
                        string[] temp = { moduleCode1, "进站1", "OK", DateTime.Now.ToString("yyyy年MM月dd日 HH:mm:ss") };
        ...
        {
            form.P_Class.TxtLog("接收极耳焊后出站信号，开始出站！", 2);
            moduleCode1 = form.PLC_ui.Read_String(FindIniPath("WIP码")[0]);
            form.P_Class.TxtLog("极耳焊后1出站WIP码：" + moduleCode1, 2);
            ...
            tempStr[DataGridViewClass.GetColumnsIndex(form.dataGridView2, "工位")] = "极耳焊后";
            ...
            readPLC_CS(tempStr, "极耳焊后", "", "", "", "", 1,ref paramsArrayList, ref materialsArrayList);
            ...
            if (!form.采集模式.Text.Contains("在线"))
            {
                form.Invoke(new MethodInvoker(delegate
                {
                    ...
                    form.P_Class.Write_CSV(form.dataGridView2, tempStr, "极耳焊后");
                }));
                ...
                string url = getData3Value("极耳焊后URL");
                string deviceCode = getData3Value("极耳焊后设备编码");
                string isOutBound = getData3Value("极耳焊后是否出站");
                string userOperator = getData3Value("极耳焊后操作人");
                string timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

                if (mesClass.outbound_MES(url, moduleCode1, deviceCode, isOutBound, userOperator, timestamp, "极耳焊后", paramsArrayList, materialsArrayList, 2, "出站1OK", "出站1NG"))
                {
                    form.Invoke(new MethodInvoker(delegate
                    {
                        string[] temp = { moduleCode1, "出站1", "OK", DateTime.Now.ToString("yyyy年MM月dd日 HH:mm:ss") };
