...
namespace DataAdministrator.自动流程
{
    class 极耳折弯滚平 : Father
    {
        public 极耳折弯滚平(Form1 form, string FilePath2)
        {
            this.form = form;
            FilePath = FilePath2;
            MES_operator = "user";
        ...
        // 信号的上升延
            form.P_Class.TxtLog("接收极耳折弯滚平进站1信号，开始进站！", 2);
            moduleCode1 = form.PLC_ui.Read_String(FindIniPath("WIP码1")[0]);
            form.P_Class.TxtLog("极耳折弯滚平1进站WIP码：" + moduleCode1, 2);
            ...
            if (!form.采集模式.Text.Contains("在线"))
            {
                ...
                string url = getData2Value("极耳折弯滚平URL");
                string deviceCode = getData2Value("极耳折弯滚平A设备编码");
                string processedMode = getData2Value("极耳折弯滚平加工模式");
                ...
                string userOperator = getData2Value("极耳折弯滚平操作人");
        ...
        {
            form.P_Class.TxtLog("接收极耳折弯滚平出站信号，开始出站！", 2);
            moduleCode1 = form.PLC_ui.Read_String(FindIniPath("WIP码1")[0]);
            form.P_Class.TxtLog("极耳折弯滚平1出站WIP码：" + moduleCode1, 2);
            ...
            tempStr[DataGridViewClass.GetColumnsIndex(form.dataGridView2, "工位")] = "极耳折弯滚平1";
            ...
            readPLC_CS2(tempStr, "极耳折弯滚平", "", "", "", "", 1,1, ref paramsArrayList, ref materialsArrayList);
            ...
            if (!form.采集模式.Text.Contains("在线"))
            {
                ...
                form.Invoke(new MethodInvoker(delegate
                {
                    ...
                    form.P_Class.Write_CSV(form.dataGridView2,tempStr, "极耳折弯滚平");
                }));
            }
            else {
                string url = getData3Value("极耳折弯滚平URL");
                string deviceCode = getData3Value("极耳折弯滚平A设备编码");
                string isOutBound = getData3Value("极耳折弯滚平是否出站");
                string userOperator = getData3Value("极耳折弯滚平操作人");
                string timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

                if (mesClass.outbound_MES_TWO(url, moduleCode1, deviceCode, isOutBound, userOperator, timestamp, "极耳折弯滚平", paramsArrayList, materialsArrayList, 1, "出站1OK", "出站1NG"))
                {
                    form.Invoke(new MethodInvoker(delegate
                    {
                        string[] temp = { moduleCode1, "出站1", "OK", DateTime.Now.ToString("yyyy年MM月dd日 HH:mm:ss") };
        ...
        public override string 工位2进站(string Code = "")
        {
            StationInTime = DateTime.Now.ToString("yyyy年MM月dd日 HH:mm:ss");
            // 信号的上升延
            form.P_Class.TxtLog("接收极耳折弯滚平进站2信号，开始进站！", 2);
            moduleCode2 = form.PLC_ui.Read_String(FindIniPath("WIP码2")[0]);
            form.P_Class.TxtLog("极耳折弯滚平2WIP码：" + moduleCode2, 2);
            ...
            if (!form.采集模式.Text.Contains("在线"))
            {
                ...
                string url = getData2Value("极耳折弯滚平URL");
                string deviceCode = getData2Value("极耳折弯滚平B设备编码");
                string processedMode = getData2Value("极耳折弯滚平加工模式");
                if (form.PLC_ui.Read_Bool(FindIniPath("NG回流模式")[0]))
                {
        ...
        {
            form.P_Class.TxtLog("接收极耳折弯滚平2出站信号，开始出站！", 2);
            moduleCode2 = form.PLC_ui.Read_String(FindIniPath("WIP码2")[0]);
            form.P_Class.TxtLog("极耳折弯滚平2出站WIP码：" + moduleCode2, 2);
            ...
            tempStr[DataGridViewClass.GetColumnsIndex(form.dataGridView2, "工位")] = "极耳折弯滚平2";
            tempStr[DataGridViewClass.GetColumnsIndex(form.dataGridView2, "进站时间")] = StationInTime;
            tempStr[DataGridViewClass.GetColumnsIndex(form.dataGridView2, "出站时间")] = DateTime.Now.ToString("yyyy年MM月dd日 HH:mm:ss");
            JArray paramsArrayList = new JArray();
            JArray materialsArrayList = new JArray();

            readPLC_CS2(tempStr, "极耳折弯滚平", "", "", "", "", 2,1, ref paramsArrayList, ref materialsArrayList);
            ...
            {
                string url = getData3Value("极耳折弯滚平URL");
                string deviceCode = getData3Value("极耳折弯滚平B设备编码");
                string isOutBound = getData3Value("极耳折弯滚平是否出站");
                string userOperator = getData3Value("极耳折弯滚平操作人");
                string timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                ...
                if (mesClass.outbound_MES_TWO(url, moduleCode2, deviceCode, isOutBound, userOperator, timestamp, "极耳折弯滚平", paramsArrayList, materialsArrayList, 2, "出站2OK", "出站2NG"))
                {
                    form.Invoke(new MethodInvoker(delegate
                    {
                        string[] temp = { moduleCode2, "出站2", "OK", DateTime.Now.ToString("yyyy年MM月dd日 HH:mm:ss") };
